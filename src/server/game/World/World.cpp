...
void World::LoadConfigSettings(bool reload)
{
    ...
    rate_values[RATE_TALENT]                            = sConfigMgr->GetFloatDefault("Rate.Talent"sv, 1.0f);
    ...
    m_int_configs[CONFIG_CHARACTERS_PER_REALM] = sConfigMgr->GetIntDefault("CharactersPerRealm"sv, MAX_CHARACTERS_PER_REALM);
    ...
    m_int_configs[CONFIG_CHARACTERS_PER_ACCOUNT] = sConfigMgr->GetIntDefault("CharactersPerAccount"sv, 50);
    ...
    m_int_configs[CONFIG_DEMON_HUNTERS_PER_REALM] = sConfigMgr->GetIntDefault("DemonHuntersPerRealm"sv, 1);
    if (int32(m_int_configs[CONFIG_DEMON_HUNTERS_PER_REALM]) < 0 || m_int_configs[CONFIG_DEMON_HUNTERS_PER_REALM] > 16)
    {
        TC_LOG_ERROR("server.loading", "DemonHuntersPerRealm ({}) must be in range 0..16. Set to 1.", m_int_configs[CONFIG_DEMON_HUNTERS_PER_REALM]);
        m_int_configs[CONFIG_DEMON_HUNTERS_PER_REALM] = 1;
    }

    m_int_configs[CONFIG_CHARACTER_CREATING_MIN_LEVEL_FOR_DEMON_HUNTER] = sConfigMgr->GetIntDefault("CharacterCreating.MinLevelForDemonHunter"sv, 70);
    ...
    m_int_configs[CONFIG_START_DEATH_KNIGHT_PLAYER_LEVEL] = sConfigMgr->GetIntDefault("StartDeathKnightPlayerLevel"sv, 55);
    ...
    m_int_configs[CONFIG_START_DEMON_HUNTER_PLAYER_LEVEL] = sConfigMgr->GetIntDefault("StartDemonHunterPlayerLevel"sv, 98);
    if (m_int_configs[CONFIG_START_DEMON_HUNTER_PLAYER_LEVEL] < 98)
    {
        TC_LOG_ERROR("server.loading", "StartDemonHunterPlayerLevel ({}) must be in range 98..MaxPlayerLevel({}). Set to 98.",
    ...
    m_int_configs[CONFIG_START_ALLIED_RACE_LEVEL] = sConfigMgr->GetIntDefault("StartAlliedRacePlayerLevel"sv, 20);
    if (m_int_configs[CONFIG_START_ALLIED_RACE_LEVEL] < 1)
    {
        TC_LOG_ERROR("server.loading", "StartAlliedRacePlayerLevel ({}) must be in range 1..MaxPlayerLevel({}). Set to 20.",
            m_int_configs[CONFIG_START_ALLIED_RACE_LEVEL], m_int_configs[CONFIG_MAX_PLAYER_LEVEL]);
        m_int_configs[CONFIG_START_ALLIED_RACE_LEVEL] = 20;
    }
    ...
    m_int_configs[CONFIG_CURRENCY_START_APEXIS_CRYSTALS] = sConfigMgr->GetIntDefault("Currency.StartApexisCrystals"sv, 0);
    ...
    m_int_configs[CONFIG_CURRENCY_MAX_APEXIS_CRYSTALS] = sConfigMgr->GetIntDefault("Currency.MaxApexisCrystals"sv, 20000);
    ...
    m_int_configs[CONFIG_CURRENCY_START_JUSTICE_POINTS] = sConfigMgr->GetIntDefault("Currency.StartJusticePoints"sv, 0);
    ...
    m_int_configs[CONFIG_CURRENCY_MAX_JUSTICE_POINTS] = sConfigMgr->GetIntDefault("Currency.MaxJusticePoints"sv, 4000);
    ...
    m_int_configs[CONFIG_CURRENCY_START_ARTIFACT_KNOWLEDGE] = sConfigMgr->GetIntDefault("Currency.StartArtifactKnowledge"sv, 55);
    if (int32(m_int_configs[CONFIG_CURRENCY_START_ARTIFACT_KNOWLEDGE]) < 0)
    {
        TC_LOG_ERROR("server.loading", "Currency.StartArtifactKnowledge ({}) must be >= 0, set to default 0.", m_int_configs[CONFIG_CURRENCY_START_ARTIFACT_KNOWLEDGE]);
    ...
    m_int_configs[CONFIG_ARENA_SEASON_ID]                            = sConfigMgr->GetIntDefault ("Arena.ArenaSeason.ID"sv, 15);
    ...
    if (int32 hotfixCacheId = sConfigMgr->GetIntDefault("HotfixCacheVersion"sv, 0))
    {
        // overwrite DB/old value
        if (hotfixCacheId > 0)
            m_int_configs[CONFIG_HOTFIX_CACHE_VERSION] = hotfixCacheId;
        else
            TC_LOG_ERROR("server.loading", "HotfixCacheVersion can't be negative {}, ignored.", hotfixCacheId);
    }

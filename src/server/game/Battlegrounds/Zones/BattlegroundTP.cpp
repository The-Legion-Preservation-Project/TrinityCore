...
bool BattlegroundTP::UpdatePlayerScore(Player* player, uint32 type, uint32 value, bool doAddHonor)
{
    ...
    switch (type)
    {
        case SCORE_FLAG_CAPTURES:
            player->UpdateCriteria(CriteriaType::TrackedWorldStateUIModified, TP_OBJECTIVE_CAPTURE_FLAG);
            break;
        case SCORE_FLAG_RETURNS:
            player->UpdateCriteria(CriteriaType::TrackedWorldStateUIModified, TP_OBJECTIVE_RETURN_FLAG);
            break;
        default:
            break;
    }
...
void BattlegroundTP::EventPlayerClickedOnFlag(Player* player, GameObject* target_obj)
{
        ...
        && BgObjects[BG_TP_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
        ...
        player->StartCriteriaTimer(CriteriaStartEvent::BeSpellTarget, BG_TP_SPELL_ALLIANCE_FLAG_PICKED);
        ...
        && BgObjects[BG_TP_OBJECT_H_FLAG] == target_obj->GetGUID())
    {
        ...
        player->StartCriteriaTimer(CriteriaStartEvent::BeSpellTarget, BG_TP_SPELL_HORDE_FLAG_PICKED);

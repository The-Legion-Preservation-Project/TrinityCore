                ...
                if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
                {
                    ...
                    if (e.action.morphOrMount.creature)
                    {
                        if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate(e.action.morphOrMount.creature))
                        {
                            ...
                            (*itr)->ToCreature()->SetDisplayId(model->CreatureDisplayID);
                ...
                if (IsUnit(*itr))
                {
                    switch (e.action.setunitByte.type)
                    {
                        ...
                        case 3:
                            ...
                            (*itr)->ToUnit()->SetAnimTier(UnitBytes1_Flags((*itr)->ToUnit()->GetByteValue(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_ANIM_TIER) | e.action.setunitByte.byte1), false);
                            break;
                    }
                }

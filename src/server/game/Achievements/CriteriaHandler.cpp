    ...
    switch (CriteriaAdditionalCondition(modifier->Type))
    {
        ...
        case CRITERIA_ADDITIONAL_CONDITION_EXPLORED_AREA: // 113
        {
            ...
            if (areaTable->AreaBit <= 0)
                break; // success
            uint32 playerIndexOffset = uint32(areaTable->AreaBit) / 32;
            if (playerIndexOffset >= PLAYER_EXPLORED_ZONES_SIZE)
                break;
            if (!(referencePlayer->GetUInt32Value(PLAYER_EXPLORED_ZONES_1 + playerIndexOffset) & (1 << (areaTable->AreaBit % 32))))
                return false;
            break;
        }
        ...
        case CRITERIA_ADDITIONAL_CONDITION_MAIN_HAND_VISIBLE_SUBCLASS: // 232
        {
            ...
            if (ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(referencePlayer->GetUInt32Value(PLAYER_VISIBLE_ITEM + VISIBLE_ITEM_ENTRY_OFFSET + (EQUIPMENT_SLOT_MAINHAND * 2))))
        ...
        {
            uint32 itemSubclass = ITEM_SUBCLASS_WEAPON_FIST_WEAPON;
            if (ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(referencePlayer->GetUInt32Value(PLAYER_VISIBLE_ITEM + VISIBLE_ITEM_ENTRY_OFFSET + (EQUIPMENT_SLOT_OFFHAND * 2))))
                itemSubclass = itemTemplate->GetSubClass();
            if (itemSubclass != reqValue)
                return false;
            break;

...
class TC_GAME_API Unit : public WorldObject
{
    public:
        ...
        void SetGender(uint8 sex) { SetByteValue(UNIT_FIELD_BYTES_0, UNIT_BYTES_0_OFFSET_GENDER, sex); }
        ...
        void HandleEmoteCommand(uint32 anim_id, Player* target = nullptr);
        ...
        bool SetCharmerData(Unit const* unit)
        {
            if (!GetCharmerGUID().IsEmpty())
                return false;
            SetGuidValue(UNIT_FIELD_CHARMEDBY, unit->GetGUID());
